-- ==========================================
-- STRIDE RITE FITNESS DASHBOARD - SECURITY UPGRADE
-- ==========================================
...
drop policy if exists "Workout exercises modification policy." on public.workout_exercises;
...
drop policy if exists "Diet meals modification policy." on public.diet_meals;
...
-- 8. WORKOUT EXERCISES POLICIES (Cascading access through parent workout)
...
create policy "Workout exercises write policy." on public.workout_exercises for all using (
  exists (
    select 1 from public.workouts w 
    where w.id = workout_exercises.workout_id 
    and (w.user_id = auth.uid() or exists (
...
-- 10. DIET MEALS POLICIES (Cascading access through parent diet log)
...
create policy "Diet meals write policy." on public.diet_meals for all using (
  exists (
    select 1 from public.diet_logs l 
    where l.id = diet_meals.diet_log_id 
    and (l.user_id = auth.uid() or exists (
