...
export default function ClientManagementPage() {
  ...
  if (loading) return <div className="p-12 text-center font-mono text-[10px] text-gray-500 animate-pulse tracking-widest">LOADING ATHLETE INTEL...</div>;
  if (!client) return <div className="p-12 text-center font-mono text-xs text-error">CRITICAL ERROR: DATA CORRUPTION</div>;

  return (
    <div className="p-6 max-w-7xl mx-auto pb-24 animate-slide-up">
      {/* Header */}
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="font-outfit text-4xl font-black tracking-tight uppercase">Athlete <span className="text-accent">Profile</span></h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card>
            ...
            <div className="mt-8 bg-background/20 p-4 rounded-xl border border-white/[0.03]">
          ...
          {exercises.length === 0 ? (
            ...
            exercises.map((exercise: any, idx: number) => (
              <div key={idx} className="bg-background/20 border border-white/[0.02] p-3.5 rounded-xl flex justify-between items-center group/item hover:border-secondary/40 transition-all duration-300 hover:bg-background/40">
                ...
                {editMode && (
                  <button onClick={() => handleRemoveExercise(exercise.id)} className="text-gray-600 hover:text-error transition-colors p-1.5 hover:bg-background/60 rounded-lg">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
      ...
      <Card className="w-full max-w-xl border-secondary/50 shadow-secondary/20 animate-slide-up">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            autoFocus
            className="w-full bg-background/60 border border-white/[0.05] rounded-xl pl-12 pr-4 py-4 font-mono text-sm uppercase tracking-widest outline-none focus:border-secondary transition-all"
          />
        </div>
    ...
    <div className="space-y-6">
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 bg-background border border-white/[0.05] rounded-xl px-4 py-3.5 font-mono text-xs uppercase tracking-widest outline-none focus:border-secondary transition-all"
        />
        <button onClick={handleAddNote} className="bg-secondary hover:bg-secondary/80 text-white px-6 rounded-xl font-outfit font-black text-xs uppercase tracking-widest transition-all duration-300">
          LOG
        </button>
      </div>
      ...
      <div className="space-y-3">
        {notes.map(note => (
          <div key={note.id} className="bg-[#0b0e14]/40 border-l-2 border-secondary p-4 rounded-r-xl">
            <p className="font-mono text-[8px] text-secondary font-bold uppercase mb-2">{new Date(note.date).toLocaleDateString()} | COMMAND</p>
