...
const muscleGroups = {
  ...
  'Biceps': {
    equipments: ['Barbell', 'Dumbbell', 'Cable', 'Machine'],
    movements: [
      { name: 'Bicep Curl', cues: 'Lock elbows to ribs, curl pinkies up, do not swing.', rationale: 'Classic biceps brachii mass builder.' },
      { name: 'Hammer Curl', cues: 'Keep thumbs up, lock elbows, curl like a hammer.', rationale: 'Targets brachialis and brachioradialis (forearm/bicep).' },
      { name: 'Preacher Curl', cues: 'Sit low, glue triceps to pad, squeeze biceps at top.', rationale: 'Maximizes isolation in stretched state.' },
      { name: 'Concentration Curl', cues: 'Anchor elbow on inner thigh, curl dumbbell to nose.', rationale: 'Isolates bicep peak contraction.' },
      { name: 'Spider Curl', cues: 'Lie prone on incline bench, let arms hang, curl upwards.', rationale: 'Eliminates shoulder swing entirely.' },
      { name: 'Bayesian Curl', cues: 'Step away from cable pulley, curl behind your torso.', rationale: 'Unbelievable stretch-mediated bicep load.' }
    ],
    variations: [
  ...
  'Triceps': {
    equipments: ['Barbell', 'Dumbbell', 'Cable', 'Machine', 'Smith Machine'],
    movements: [
      { name: 'Skullcrusher', cues: 'Lower bar to forehead or behind head, flare elbows in.', rationale: 'Ultimate long-head tricep stretch.' },
      ...
      { name: 'Tricep Pushdown', cues: 'Tuck elbows, lock chest, drive hands down to fully extend.', rationale: 'High mechanical advantage tricep isolator.' },
      { name: 'Kickback', cues: 'Keep upper arm parallel to floor, extend elbow fully.', rationale: 'Peak contraction for lateral and medial heads.' },
  ...
  'Calves': {
    ...
    movements: [
      { name: 'Calf Raise', cues: 'Get a full stretch at bottom, pause 1s, stand on toes, pause 1s.', rationale: 'Essential gastrocnemius/soleus builder.' },
      { name: 'Donkey Calf Raise', cues: 'Bend at hips, raise calves with load on back, maximize stretch.', rationale: 'Hits the upper calf gastroc head with deep stretch.' }
    ],
    variations: [
  ...
  'Abs': {
    ...
    movements: [
      { name: 'Crunch', cues: 'Tuck ribs to hips, squeeze abs, do not pull head.', rationale: 'Key upper ab flexor.' },
      { name: 'Leg Raise', cues: 'Keep legs straight, tilt pelvis up, raise legs past 90.', rationale: 'Outstanding lower ab movement.' },
      { name: 'Plank', cues: 'Keep body straight like a board, squeeze glutes and abs.', rationale: 'Core static stabilization.' },
      { name: 'Russian Twist', cues: 'Sit at 45 angle, rotate shoulders side to side with load.', rationale: 'Hits obliques.' },
      { name: 'Woodchopper', cues: 'Rotate cable diagonally across body, pivot back foot.', rationale: 'Rotational oblique strength.' },
      { name: 'Ab Wheel Rollout', cues: 'Brace core, roll out as far as possible, pull back with abs.', rationale: 'Superior eccentric core builder.' }
    ],
    variations: [
...
// Let's generate 850+ distinct exercises by cross-multiplying everything!
  ...
  for (const eq of data.equipments) {
    for (const mov of data.movements) {
      for (const varName of data.variations) {
        ...
        // Formulate a beautiful, natural exercise name
        // e.g. "Seated Incline Dumbbell Bench Press" or "Single-Arm Cable Row"
        let name = `${varName} ${eq} ${mov.name}`;
        
        // Standardize naming slightly for premium readability
        ...
        // Also add standard combinations
...
const valueRows = exercises.slice(0, 850).map((ex, idx) => {
...
console.log(`Successfully generated seed_exercises_800.sql with ${valueRows.length} distinct exercises!`);
